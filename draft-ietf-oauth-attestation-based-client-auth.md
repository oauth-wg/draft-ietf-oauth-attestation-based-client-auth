...
- optionally, further characteristics of the Client Instance (e.g., the criteria or policy against which the Client Instance was evaluated)
...
:  A JSON Web Token (JWT) generated by the Client Attester that attests to the authenticity of a Client Instance and is cryptographically bound to a key managed by that Client Instance. A Client Attestation JWT may additionally convey information about the integrity, state, or other characteristics of the Client Instance.
...
1. If a `client_id` is provided in the request containing the Client Attestation, then this `client_id` matches the `sub` claim of the Client Attestation JWT, unless specified otherwise by a profile as described in [](#profiling). See [](#client-attestation-as-client-auth) for how this parameter is used when the Client Attestation is used as an OAuth Client Authentication mechanism.
...
## Trust Management and Key Resolution {#trust-management}
